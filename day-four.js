...
function validatePassports() {
  ...
  for (let i = 0; i < passports.length; i++) {
    const line = passports[i].split(' ');
    if (line[0] === EMPTY_PASS) {
      if (validateKeys(auxKeyPassport)) {
        validPassports++;
      }
      auxKeyPassport = [];
    }
    for (let j = 0; j < line.length; j++) {
      if (line[j] !== EMPTY_PASS) {
        auxKeyPassport.push(line[j].substring(0,line[j].indexOf(':')))
      }
    }
  }
...
function validateKeys(arrKeys) {
  let valid = true;
  const validFields = VALID_FIELDS;
  for (const el of validFields) {
    if (!arrKeys.includes(el)) {
      valid = false;
      break;
    } else {
      continue;
  ...
  if (valid) {
    console.log("VALIDO: ", arrKeys);
  } else {
    console.log("NO VALIDO: ", arrKeys);
  }
  
  return valid;
}

function validatePassports2() {
  const passports = INPUT;
  let validPassports = 0;
  let auxKeyPassport = [];
  ...
  for (let i = 0; i < passports.length; i++) {
    const line = passports[i].split(' ');
    if (line[0] === EMPTY_PASS) {
      if (validateKeys2(auxKeyPassport)) {
        if (validateValues(auxKeyPassport)) {
          validPassports++;
        }
      }
      auxKeyPassport = [];
    ...
    for (let j = 0; j < line.length; j++) {
      if (line[j] !== EMPTY_PASS) {
        const auxObj = {
          key: line[j].substring(0,line[j].indexOf(':')),
          value: line[j].substring(line[j].indexOf(':')+1),
        }
        auxKeyPassport.push(auxObj);
      }
    }
  }
...
function validateKeys2(arrKeys) {
  let valid = true;
  const validFields = VALID_FIELDS;
  for (const el of validFields) {
    const found = arrKeys.find(i => {
      return i.key === el;
    });
    if (!found) {
      valid = false;
...
function validateValues(arrKeys) {
  for (const el of arrKeys) {
    switch (el.key) {
      ...
      case 'hgt': {
        if (!el.value.endsWith('cm') && !el.value.endsWith('in')) {
          return false;
        }
        if (el.value.endsWith('cm')) {
          const num = parseInt(el.value.substring(0,el.value.indexOf('c')));
          if(num < 150 || num > 193) {
            return false;
          } else {
            break;
        ...
        if (el.value.endsWith('in')) {
          const num = parseInt(el.value.substring(0,el.value.indexOf('i')));
          if(num < 59 || num > 76) {
            return false;
          } else {
            break;
      ...
      case 'ecl': {
        if (!VALID_COLORS.includes(el.value)) {
          return false;
        }
        break;
      }
...
console.log(validatePassports2());